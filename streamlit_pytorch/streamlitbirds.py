...
def load_model(model_path):
    model = models.efficientnet_b2(pretrained=False)  # Base architecture
    ...
    model.classifier = nn.Sequential(
        ...
        nn.SiLU(),
        nn.Dropout(0.3),
        nn.Linear(2048, len(classes))  # Number of output classes
    )
...
# Load the model
model_path = model_path = "C:/Users/Aditi/Downloads/birds_streamlit.pth"
...
# Streamlit UI
st.title("Image Classification with EfficientNet-B2")
...
if uploaded_file is not None:
    ...
    st.write(f"Predicted Class: {predicted_label[4:]}")
